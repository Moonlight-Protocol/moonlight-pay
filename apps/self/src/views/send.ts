...
import { selfSend } from "shared/api/client.ts";
...
function renderContent(): HTMLElement {
  ...
  el.innerHTML = `
    ...
    <p style="color:var(--text-muted);margin-bottom:1.5rem">
      Send XLM privately to any Stellar address. If the receiver has completed KYC,
      funds go directly to their UTXOs. Otherwise, funds are held in escrow until they verify.
    </p>
  ...
  sendBtn.addEventListener("click", async () => {
    ...
    try {
      const { data } = await selfSend(to, stroops);

      capture("pay_self_send", { to: to.slice(0, 8), amount: stroops });
